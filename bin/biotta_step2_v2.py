...
def run_tta_and_predict(
    ...
    # 转换到原始图像坐标系
    # 需要根据extract_brain的逆变换
    # 这里简化处理，实际可能需要更复杂的坐标变换
    # TODO: 实现完整的坐标变换
    
    # 保存landmarks.txt（如果配置允许）
...
# 未调用下面两个结果处理函数，函数的绘图效果也不全面（y轴范围设置有误、没有原图坐标结果、不支持深色/浅色双模式、 分位数计算方法为线性计算）

def calculate_lengths_and_centiles(landmarks, ga, curve_dir, output_dir):
    """
    计算11对点的距离和对应的分位数
    ...
    Returns:
        lengths_df: 包含长度和分位数的DataFrame
    """
    # 生物测量名称
    biometry = [
        "RLV-A", "LLV-A", "RLV-C", "LLV-C", "BBD", "CBD", 
        "TCD", "FOD", "AVD", "VH", "CCL"
    ]
    ...
    for i in range(11):
        ...
        # 计算欧氏距离并乘以分辨率
        distance = np.sqrt(np.sum((point1 - point2)**2)) * resolution
        lengths.append(distance)
        
        # 读取对应的CSV文件
        csv_file = os.path.join(curve_dir, f"{biometry[i]}.csv")
        if os.path.exists(csv_file):
            df = pd.read_csv(csv_file)
            
            # 找到最接近的胎龄行
            ga_diff = np.abs(df['GA'] - ga)
            closest_idx = ga_diff.idxmin()
            closest_row = df.iloc[closest_idx]
            
            # 计算分位数
            length_50th = closest_row['50th']
            length_5th = closest_row['5th']
            length_95th = closest_row['95th']
            
            if distance <= length_5th:
                centile = 0.05 * (distance / length_5th)
            elif distance >= length_95th:
                centile = 0.95 + 0.05 * ((distance - length_95th) / length_95th)
            else:
                # 线性插值计算分位数
                centile = 0.05 + 0.9 * ((distance - length_5th) / (length_95th - length_5th))
                
            centiles.append(centile)
        else:
            centiles.append(np.nan)
    ...
    # 保存到CSV
    output_csv = os.path.join(output_dir, 'length_centile.csv')
    results_df.to_csv(output_csv, index=False)
    print(f"✓ 长度和分位数已保存到: {output_csv}")
    
    return results_df


def plot_centile_curves(ga, lengths_df, curve_dir, output_dir):
    """
    绘制11个长度的分位数曲线图
    ...
    # 设置y轴范围
    y_axis_settings = {
        "RLV-A": (0, 50, 10), "LLV-A": (0, 50, 10),
        "RLV-C": (0, 50, 10), "LLV-C": (0, 50, 10),
        "BBD": (0, 50, 10), "CBD": (0, 50, 10),
        "TCD": (0, 50, 10), "FOD": (0, 50, 10),
        "AVD": (0, 50, 10), "VH": (0, 50, 10),
        "CCL": (0, 50, 10)
    }
    ...
    for i, length_name in enumerate(biometry):
        ax = axes_flat[i]
        ...
        # 网格设置
        ax.grid(True, linestyle="-", linewidth=1.5, color="gray", alpha=0.5)
        ax.tick_params(axis="both", direction="in", width=1.5, length=6)
        ax.tick_params(axis="x", labelsize=18, pad=10)
        ax.tick_params(axis="y", labelsize=18)
        
        # 读取曲线数据
        curve_path = os.path.join(curve_dir, f"{length_name}.csv")
        ...
        # 绘制当前case的散点
        current_length = lengths_df.loc[lengths_df['Biometry'] == length_name, 'Length_mm'].values[0]
        ax.scatter(
            [ga],
            [current_length],
            color="royalblue",
        ...
        if length_name in ["RLV-A", "TCD"]:
            ax.set_ylabel("Length (mm)", fontsize=22)
        if length_name in ["TCD", "FOD", "AVD", "VH", "CCL"]:
            ax.set_xlabel("Gestational Age (weeks)", fontsize=22)
    
    # 隐藏第12个多余子图
    if len(axes_flat) > len(biometry):
        axes_flat[len(biometry)].set_visible(False)
    
    plt.tight_layout()
    
    # 保存图像
    output_png = os.path.join(output_dir, 'length_centile.png')
    plt.savefig(output_png, bbox_inches="tight", dpi=300)
    plt.close()
    print(f"✓ 分位数曲线图已保存到: {output_png}")
